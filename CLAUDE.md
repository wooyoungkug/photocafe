...
> - 🚨 **레거시 NAS 포트 차단 필요** (아래 보안 경고 참조)
> - 단계별 절차는 `docs/INFRASTRUCTURE.md` §7. 배포 로드맵 참조.

### 🚨 보안 경고 — 레거시 NAS 잔존 노출 (즉시 조치 필요)

외부 포트 스캔 결과, **공인 IP `1.212.201.147`** 에서 다음 포트가 여전히 인터넷에 열려 있습니다:

| 포트 | 서비스 | 위험도 | 즉시 조치 |
|------|--------|--------|-----------|
| **5433** | **PostgreSQL** | 🔴 **심각** | **Synology 방화벽/공유기 포트포워딩에서 즉시 차단** |
| 3000 | 옛 Next.js 프론트 (200 OK 응답 중) | 🟠 높음 | 컨테이너 종료 또는 방화벽 차단 |
| 3001 | 옛 NestJS API (Helmet 헤더 응답 중) | 🟠 높음 | 컨테이너 종료 또는 방화벽 차단 |
| 22 | SSH | 🟡 중간 | 키 인증 전용 + IP allowlist + fail2ban 권장 |

조치 가이드:
- `## 운영 환경 운영/관제` → `### Synology NAS (Legacy, 잔존 시)` 섹션 (단계별 차단 명령)
