...
**관리자 컨텍스트로 인식되는 URL prefix** ([auth-store.ts:81-95](../../apps/web/stores/auth-store.ts) `isAdminContext()`):

```
/dashboard /admin /company /product /order /production /pricing
/schedule /master /accounting /cs /settings /statistics
```

> ⚠️ 동일 목록이 [`api.ts:14-33`](../../apps/web/lib/api.ts), [`middleware.ts`](../../apps/web/middleware.ts) `ADMIN_PATHS`, [`auth.controller.ts:65-79`](../../apps/api/src/modules/auth/auth.controller.ts)에도 존재한다. **새 관리자 경로 추가 시 4곳 모두 동기화** 필요. (단일화는 후속 리팩터링.)
...
| L2 | ADMIN 경로 목록 4곳 중복 정의 | auth-store.ts / api.ts / middleware.ts / auth.controller.ts | ⏳ 단일화 미정 |
...
- [ ] 새 관리자 경로 추가 시 [auth-store.ts:81-95](../../apps/web/stores/auth-store.ts), [api.ts:14-33](../../apps/web/lib/api.ts), [middleware.ts](../../apps/web/middleware.ts) `ADMIN_PATHS`, [auth.controller.ts:65-79](../../apps/api/src/modules/auth/auth.controller.ts) — **4곳 모두** 추가했는가?
