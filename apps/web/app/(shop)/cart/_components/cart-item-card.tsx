...
function CartItemThumbnail({ item }: { item: CartItem }) {
  ...
  const isLeftStart = item.albumOrderInfo?.bindingDirection?.includes('LEFT_START');
  const needsCrop = isSpread && (isRightStart || isLeftStart);

  // 펼친면: 좌시작→왼쪽 1p만, 우시작→오른쪽 1p만 크롭
  useEffect(() => {
    ...
    img.onload = () => {
      ...
      if (ctx) {
        // 우시작: 오른쪽 절반, 좌시작: 왼쪽 절반
        const srcX = isRightStart ? halfWidth : 0;
        ctx.drawImage(
          img,
          srcX, 0, halfWidth, srcHeight,        // source: 해당 절반
                ...
                {item.albumOrderInfo && (
                  <div className="mt-1.5">
                    <div className="inline-flex items-center gap-1 text-[11px] text-black/80 bg-purple-50 rounded-md px-2 py-1 flex-wrap">
                      <span>
                        {item.albumOrderInfo.printMethod === 'indigo' ? '인디고출력' : '잉크젯'}
                      </span>
