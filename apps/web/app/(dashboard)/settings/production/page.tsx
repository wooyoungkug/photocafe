...
import { PrintAgentStatusStrip } from '@/app/(dashboard)/orders/print-queue/components/print-agent-status';
...
export default function ProductionSettingsPage() {
  ...
  const handleSave = () => {
    ...
    bulkUpdate.mutate(settings, {
      onSuccess: () => {
        toast.success('생산 설정이 저장되었습니다.');
      },
    });
  };

  return (
    <div className="space-y-4 sm:space-y-5 max-w-[800px]">
      {/* 헤더 */}
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-[24px] text-black font-normal">생산 설정</h1>
          <p className="text-[14px] text-gray-500 mt-0.5">
            JDF+PDF 출력, 인덱스 표기, 임포지션 기본 마크, 작업지시서 자동 인쇄 등 생산 단계 전반의 설정
          </p>
        </div>
        <Link
          href="/orders/print-queue"
          className="text-[14px] text-gray-500 hover:text-black underline"
        >
          출력대기로 →
        </Link>
      </div>
      ...
      <div className="flex items-center justify-end gap-2 pt-2 pb-6">
        <Button
          ...
          onClick={() => router.push('/settings/imposition-presets')}
        >
          임포지션 프리셋 관리 →
        </Button>
